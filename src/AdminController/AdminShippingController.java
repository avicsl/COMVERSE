...
public class AdminShippingController {
    ...
    @FXML
    private TextField shippingaddressTF;
    ...
    public void initialize() {
        ...
        shippingaddressTF.setPromptText("Enter new shipping address");
    }
    ...
    private void setupButtonActions() {
        updatehistoryBTN.setOnAction(e -> handleUpdateShipping());
        deletehistorybtn.setOnAction(e -> handleDeleteShipping());
    }

    private void handleUpdateShipping() {
        ShippingData selectedShipping = mytable.getSelectionModel().getSelectedItem();
        
        if (selectedShipping == null) {
            showAlert("No Selection", "Please select a shipping record to update.");
            return;
        }
        
        String newAddress = shippingaddressTF.getText().trim();
        
        if (newAddress.isEmpty()) {
            showAlert("Invalid Address", "Please enter a new shipping address.");
            return;
        }
        
        if (!newAddress.equals(selectedShipping.getShippingAddress())) {
            updateShippingAddress(selectedShipping.getShippingId(), newAddress);
        } else {
            showAlert("No Change", "The entered address is the same as the current one.");
        }
    }

    private void updateShippingAddress(String shippingId, String newAddress) {
        String query = "UPDATE shipping SET shipping_address = ?, shipping_date = CURDATE() WHERE shipping_id = ?";
        ...
        try (Connection conn = DatabaseHandler.getDBConnection();
            ...
            if (rowsAffected > 0) {
                ...
                shippingaddressTF.clear();
                loadShippingData(); // Refresh the table
            } else {
                showAlert("Error", "No shipping record found to update.");
            }
            
        } catch (SQLException e) {
            e.printStackTrace();
            showAlert("Database Error", "Could not update shipping address: " + e.getMessage());
        }
    }
