...
public class AdminPaymentController {
    ...
    private void setupPaymentMethodComboBox() {
        ...
        // Don't set a default value - let admin choose
    }
    ...
    private void setupButtonActions() {
        updatepaymentbtn.setOnAction(e -> handleUpdatePayment());
    ...
    private void handleUpdatePayment() {
        ...
        if (selectedPayment == null) {
            showAlert("No Selection", "Please select a payment record to update.");
            return;
        }
        ...
        if (selectedPaymentMethod == null || selectedPaymentMethod.isEmpty()) {
            showAlert("No Payment Method Selected", "Please select a payment method from the dropdown to update.");
            return;
        }
        
        if (!selectedPaymentMethod.equals(selectedPayment.getPaymentMethod())) {
            updatePaymentMethod(selectedPayment.getPaymentId(), selectedPaymentMethod);
        } else {
            showAlert("No Change", "The selected payment method is the same as the current one.");
        }
    }

    private void updatePaymentMethod(String paymentId, String newPaymentMethod) {
        ...
        try (Connection conn = DatabaseHandler.getDBConnection();
            ...
            if (rowsAffected > 0) {
                ...
                showAlert("Error", "No payment record found to update.");
            }
            
        } catch (SQLException e) {
            e.printStackTrace();
            showAlert("Database Error", "Could not update payment method: " + e.getMessage());
        }
    }
